...
let markerEnd = {
   ...
   draw() {
      if (!ballsMoving || (balls[0] && !balls[0].sliding)) return;
      ...
      for (let i = 0; i < squares.length; i++) {
         for (let j = 0; j < squares[i].length; j++) {
            ...
            function fixPos(corner, thisBall) {
               ...
               const b = -2 * corner.x * thisBall.vx + 2 * thisBall.x * thisBall.vx - 2 * corner.y * thisBall.vy + 2 * thisBall.y * thisBall.vy;
               const c =
                  corner.x ** 2 -
