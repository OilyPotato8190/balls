  ...
  checkCollision() {
    ...
    for (let i = 0; i < objects.squares.length; i++) {
      ...
      function cornerBounce(corner, thisBall) {
        const normal = tangentVector(thisBall, corner);
        const vector = { x: thisBall.vx, y: thisBall.vy };
        let dotProduct = getDotProduct(vector, normal);
        let reflection = subtractVectors(vector, scaleVector(normal, 2 * dotProduct));
        console.log(reflection);

        function tangentVector(circle, point) {
          const radiusVector = { x: point.x - circle.x, y: point.y - circle.y };
          const radiusUnitVector = { x: radiusVector.x / circle.r, y: radiusVector.y / circle.r };
          const tangentUnitVector = { x: -radiusUnitVector.y, y: radiusUnitVector.x };
          const tangentVector = { x: tangentUnitVector.x * circle.r, y: tangentUnitVector.y * circle.r };
